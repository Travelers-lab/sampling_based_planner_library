...
def test_rrt_connect(cost_map, start_goal):
    ...
    planner = RRTConnect(step_size=5.0, max_iterations=500, sampling_method=LearningBasedSampler())
...
def test_bidirectional_est(cost_map, start_goal):
    start, goal = start_goal
    planner = BidirectionalEST(step_size=5.0, max_iterations=500, sampling_method=LearningBasedSampler())
    path = planner.plan(start, goal, cost_map, {'length': 0.7, 'cost': 0.3})
    assert path is None or (path[0] == start and path[-1] == goal)

def test_bit_star(cost_map, start_goal):
    start, goal = start_goal
    planner = BITStar(batch_size=50, max_batches=5, sampling_method=LearningBasedSampler())
    path = planner.plan(start, goal, cost_map, {'length': 0.7, 'cost': 0.3})
    assert path is None or (path[0] == start and path[-1] == goal)

def test_prm(cost_map, start_goal):
    start, goal = start_goal
    planner = PRM(n_samples=100, k_neighbors=5, sampling_method=LearningBasedSampler())
    path = planner.plan(start, goal, cost_map, {'length': 0.7, 'cost': 0.3})
    assert path is None or (path[0] == start and path[-1] == goal)
